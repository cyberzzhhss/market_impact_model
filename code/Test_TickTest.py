...
class Test_TickTest(unittest.TestCase):
    ...
    def test_classifyAll(self):
        filePathName = baseDir = os.path.dirname(os.getcwd()) + "/trades/20070620/IBM_trades.binRT"
        ...
        self.assertAlmostEqual(classifications[-3][1], classifications[-2][1],
                               0.00001)  # 3rd last price and 2nd last price are the same
        self.assertAlmostEqual(classifications[-2][1], classifications[-1][1],
                               0.00001)  # 2nd last price and last price are the same
